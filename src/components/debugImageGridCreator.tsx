...
export default function DebugImageGridCreator() {
  ...
  const handleProcessImage = async () => {
    ...
    try {
      ...
      // 인스타그램 프리뷰 이미지 생성 (좌우 16픽셀 제외)
      const previewBlob = await createGridPreview(images, gridType);
      const previewUrl = URL.createObjectURL(previewBlob);
      setPreviewImage(previewUrl);
      
      // 원본 프리뷰 이미지 생성 (좌우 16픽셀 포함)
      const originalPreviewBlob = await createGridPreview(images, gridType);
      const originalPreviewUrl = URL.createObjectURL(originalPreviewBlob);
      setOriginalPreviewImage(originalPreviewUrl);
    } catch (error) {
...
감사합니다!`;
  ...
  const [previewMode, setPreviewMode] = useState<'instagram' | 'original'>('instagram');
    ...
    <div className="container mx-auto p-4">
      ...
      {originalImageInfo && (
        <div className="mb-4 bg-blue-50 p-3 rounded border border-blue-100">
          ...
          <p className="text-blue-700 text-sm">
            중복 영역: 좌우 각각 16픽셀씩 추가 (인스타그램 피드에서는 제거됨)
          </p>
        </div>
      )}
      ...
      {previewImage && (
        <div className="mb-6">
          <h2 className="text-xl font-bold mb-2">프리뷰</h2>
          
          <div className="flex space-x-4 mb-2">
            <button 
              onClick={() => setPreviewMode('instagram')}
              className={`px-3 py-1 rounded text-sm ${previewMode === 'instagram' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
            >
              인스타그램 피드 모습
            </button>
            <button 
              onClick={() => setPreviewMode('original')}
              className={`px-3 py-1 rounded text-sm ${previewMode === 'original' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
            >
              원본 (좌우 16픽셀 포함)
            </button>
          </div>
          ...
          <div className="border p-2 bg-gray-100 rounded relative">
            {previewMode === 'instagram' && previewImage && (
              <>
                <img 
                  src={previewImage} 
                  alt="인스타그램 피드 프리뷰" 
                ...
                <p className="text-center text-sm mt-2 text-gray-600">
                  인스타그램 피드에서 보이는 모습 (좌우 16픽셀 제외)
                </p>
              </>
            )}
            
            {previewMode === 'original' && originalPreviewImage && (
              <>
                <img 
                ...
                {showDebugLines && (
                  <div className="absolute inset-0 pointer-events-none">
                    {/* 수직 그리드 라인 */}
                    <div className="absolute left-1/3 top-0 bottom-0 border-l-2 border-blue-500 border-dashed opacity-50"></div>
                    <div className="absolute left-2/3 top-0 bottom-0 border-l-2 border-blue-500 border-dashed opacity-50"></div>
                    
                    {/* 수평 그리드 라인 - 그리드 타입에 따라 다름 */}
                    {gridType !== '3x1' && (
                      <>
                        <div className="absolute left-0 right-0 top-1/2 border-t-2 border-blue-500 border-dashed opacity-50"></div>
                        {gridType === '3x3' && (
                          <>
                            <div className="absolute left-0 right-0 top-1/3 border-t-2 border-blue-500 border-dashed opacity-50"></div>
                            <div className="absolute left-0 right-0 top-2/3 border-t-2 border-blue-500 border-dashed opacity-50"></div>
                          </>
                        )}
                      </>
                    )}
